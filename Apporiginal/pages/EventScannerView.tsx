...
const EventScannerView: React.FC = () => {
  ...
  const demoSimulateScan = () => {
     handleFoundCode("A-101"); // Simulating a valid code for demo
  };
                ...
                {cameraPermission === false ? (
                    ...
                    <>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center z-10 pointer-events-none">
                         ...
                         {/* Demo Button (Pointer events allowed) */}
                        <div className="absolute bottom-24 left-0 right-0 flex justify-center z-20">
                            <button onClick={demoSimulateScan} className="text-xs text-white/20 border border-white/10 px-2 py-1 rounded hover:bg-white/10">
                                [Demo: Simular A-101]
                            </button>
                        </div>
                    </>
        ...
        <div className="w-full max-w-md p-6 z-20 relative">
                ...
                <div className="flex justify-center mb-6">
                    <div className="bg-black/40 backdrop-blur-md p-1 rounded-full flex border border-white/10">
                        <button 
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${isScanning ? 'bg-lumina-accent text-white shadow-lg' : 'text-white/50'}`}
                        >
                            Cámara
                        </button>
                        <button 
                ...
                <GlassCard className="p-8 animate-fade-in-up">
                    <form onSubmit={handleManualSubmit} className="flex flex-col gap-4">
                        ...
                        <input 
                            ...
                            placeholder="A-..."
                            autoFocus
                            className="w-full bg-black/40 border border-white/20 rounded-xl p-6 text-center text-4xl font-mono font-bold text-white focus:border-lumina-accent outline-none uppercase"
                        />
            ...
            {scannedOrder && (
                <div className="animate-fade-in-up">
                    <GlassCard className="p-0 border-lumina-accent overflow-hidden shadow-[0_0_50px_rgba(139,92,246,0.4)]">
                        {/* Header */}
                        <div className="bg-lumina-accent p-6 text-center relative overflow-hidden">
                             <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20"></div>
                             <h2 className="text-6xl font-black text-white font-mono relative z-10">{scannedOrder.pickupCode}</h2>
                             <p className="text-white/80 font-bold mt-2 relative z-10">{scannedOrder.customerName}</p>
                        </div>
