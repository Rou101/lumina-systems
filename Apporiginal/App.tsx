...
import StaffDashboard from './pages/StaffDashboard'; // Renamed
import KitchenDisplay from './pages/KitchenDisplay';
import AdminSettings from './pages/AdminSettings';
import Login from './pages/Login';
import { LandingPage } from './components/LandingPage';
import CorporateWeb from './pages/CorporateWeb'; // New Page
      ...
      <Routes>
        <Route path="login" element={
            <Login onLogin={(r) => { 
                ...
                if(r === 'kitchen') navigate('/staff/kitchen');
                if(r === 'admin') navigate('/staff/admin');
            }} />
        } />
        {/* New Staff Dashboard replacing Waiter Dashboard */}
    ...
    <HashRouter>
      <div className="font-sans antialiased selection:bg-cyan-500 selection:text-black">
        <Routes>
          ...
          {/* Corporate Web (B2B & Data) */}
          <Route path="/corporate" element={<CorporateWeb />} />
          
          {/* Main Event Client View (The Product) */}
          <Route path="/event/:tableId" element={<EventClientView />} />
          
          {/* Legacy Restaurant Redirect (Can be removed later) */}
          <Route path="/menu/:tableId" element={<Navigate to="/event/GEN" />} />
