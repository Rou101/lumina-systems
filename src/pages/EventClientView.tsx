        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center">
            ...
            <div className="bg-[#1C1C1E] w-full max-w-md rounded-t-[20px] p-6 relative z-10 animate-fade-in-up pb-safe-bottom">
                ...
                <div className="flex justify-between items-end mb-8">
                    <span className="text-gray-400 text-sm font-medium">{t('client_total_pay')}</span>
                ...
                >
                    {processing ? (
                        <>
                            ...
                            <span>{t('client_processing')}</span>
                        </>
                    ) : (
                        <>
                            <IconApplePay />
                            <span>{t('client_pay')}</span>
                        </>
                    )}
                </button>
...
const DigitalTicket: React.FC<{ order: Order, onNewOrder: () => void }> = ({ order, onNewOrder }) => {
    ...
    return (
        <div className="fixed inset-0 bg-[#050505] z-40 flex flex-col items-center justify-center p-6 animate-fade-in-up">
            ...
            <h1 className="text-3xl font-black text-white uppercase tracking-tighter mb-2">{t('client_order_confirmed')}</h1>
            <p className="text-lumina-text-secondary text-sm font-mono mb-10">{t('client_order_subtitle')}</p>
            ...
            <div className="w-full max-w-sm bg-white text-black rounded-sm overflow-hidden relative shadow-2xl">
                ...
                <div className="p-8 text-center border-b border-dashed border-gray-300">
                    <p className="text-xs font-bold uppercase text-gray-400 mb-1">{t('client_pickup_code')}</p>
                ...
                <div className="p-8 bg-gray-50 flex flex-col items-center">
                    ...
                    <p className="text-[10px] text-gray-500 uppercase font-mono">{t('client_show_qr')}</p>
                </div>
            </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1 mt-0.5">
                                <IconLock />
                                <span className="text-[9px] font-mono text-emerald-400 uppercase tracking-widest">{t('client_secure_tunnel')}</span>
                            </div>
                        </div>
                    </div>
                    <div className="bg-lumina-panel border border-lumina-border px-3 py-1 rounded-full flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                        <span className="text-[10px] font-bold text-lumina-text-secondary uppercase">{t('client_live')}</span>
                    </div>
                </div>
                            ...
                            <div key={product.id} className={`bg-lumina-panel border border-lumina-border rounded-sm overflow-hidden flex relative ${isOutOfStock ? 'opacity-50' : ''}`}>
                                ...
                                {isOutOfStock && (
                                    <div className="absolute inset-0 z-20 bg-black/60 flex items-center justify-center backdrop-blur-[2px]">
                                        <span className="text-white font-mono font-bold border border-white/50 px-3 py-1 text-xs uppercase tracking-widest">{t('client_sold_out')}</span>
                                    </div>
                                )}
                ...
                <div className="fixed bottom-6 left-4 right-4 z-40 animate-fade-in-up">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-white text-xs font-bold uppercase tracking-widest">{t('client_view_bag')}</span>
                        </div>
                        <div className="flex items-center gap-2">
